...
Dynamic tag selection based on detected technologies:
  - Base tags always: vuln, cve, discovery, vkev, panel, xss, exposure, osint
...
class NucleiScanner:
    ...
    def scan(self, alive_hostnames: List[str], detected_techs: Set[str],
             rate_limit: int = 150, bulk_size: int = 25,
             concurrency: int = 25) -> List[NucleiResult]:
        """
        Run nuclei against alive subdomains.

        Args:
            alive_hostnames: List of alive subdomain hostnames.
            detected_techs: Set of technology names from httpx/tech profiler.
            rate_limit: Requests per second limit.
            bulk_size: Number of hosts per batch.
        ...
        # Build dynamic tags
        tags = self.build_tags(detected_techs)
        self.stats.tags_used = tags
        self.stats.hosts_scanned = len(alive_hostnames)

        # Write targets to temp file — bare hostnames (no scheme prefix!)
        # nuclei handles protocol discovery itself; adding https:// breaks
        # many templates that need to probe multiple ports/protocols.
        tmpdir = tempfile.mkdtemp(prefix="reconx_nuclei_")
        input_file = os.path.join(tmpdir, "targets.txt")
        output_file = os.path.join(tmpdir, "results.jsonl")
        ...
        try:
            ...
            # Run nuclei — stdout shows findings in real-time,
            # stderr shows progress/stats, -je writes JSON to file
            timeout_secs = max(600, len(alive_hostnames) * 5)
            proc = subprocess.Popen(
                cmd,
                stdout=sys.stderr,        # Show findings on terminal (via stderr)
                stderr=sys.stderr,        # Show stats/progress on terminal
            )
            ...
            # Parse JSONL results from the -je export file
            ...
            # Cleanup temp files
            try:
                for fp in [input_file, output_file]:
