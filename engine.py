...
class ReconEngine:
    ...
    def run(self) -> ScanResult:
        ...
        # ── Phase 9d: RDP brute-force with CME-extracted usernames ───────
        if (self.result.cme_available
                and self.result.nmap_available
                and self.result.nmap_results):
            cme_rdp_users = self.cme_scanner.get_rdp_usernames()
            if cme_rdp_users:
                all_cme_users = sorted(set(
                    name for names in cme_rdp_users.values() for name in names
                ))
                already_tested = {"Administrator", "Guest"}
                new_users = [u for u in all_cme_users if u not in already_tested]

                if new_users:
                    print(
                        f"\033[36m[>]\033[0m rdp-brute: extracted "
                        f"\033[96m{len(new_users)}\033[0m username(s) from CME RDP \u2192 "
                        f"\033[96m{', '.join(new_users[:5])}\033[0m"
                        f"{'...' if len(new_users) > 5 else ''}"
                    )

                    rdp_cme_scanner = RDPBruteScanner(self.config.scanner)
                    rdp_cme_output_dir = os.path.join(".", domain)
                    os.makedirs(rdp_cme_output_dir, exist_ok=True)

                    rdp_cme_results = self._safe_scan(
                        "rdp-brute (CME users)", rdp_cme_scanner.scan,
                        self.result.nmap_results, output_dir=rdp_cme_output_dir,
                        users=new_users,
                    )

                    if rdp_cme_results is not None:
                        rdp_cme_stats = rdp_cme_scanner.stats

                        # Merge credentials into existing RDP results
                        if not self.result.rdp_results:
                            self.result.rdp_results = {}
                        for ip, hr in rdp_cme_results.items():
                            if ip in self.result.rdp_results:
                                existing_creds = {
                                    (c.username, c.password)
                                    for c in self.result.rdp_results[ip].credentials
                                }
                                for cred in hr.credentials:
                                    if (cred.username, cred.password) not in existing_creds:
                                        self.result.rdp_results[ip].credentials.append(cred)
                                self.result.rdp_results[ip].users_tested.extend(hr.users_tested)
                            else:
                                self.result.rdp_results[ip] = hr

                        # Recompute merged RDP stats
                        all_creds = []
                        all_tested = 0
                        for r in self.result.rdp_results.values():
                            all_creds.extend(r.credentials)
                            all_tested += len(r.users_tested)
                        self.result.rdp_stats.update({
                            "credentials_found": len(all_creds),
                            "pwned_count": sum(1 for c in all_creds if c.pwned),
                            "total_users_tested": all_tested,
                        })
                        self.result.rdp_available = True
                        self._save_phase("rdp")

                        if rdp_cme_stats.credentials_found > 0:
                            pwn_str = ""
                            if rdp_cme_stats.pwned_count > 0:
                                pwn_str = (
                                    f" | \033[1;91m{rdp_cme_stats.pwned_count} Pwn3d!\033[0m"
                                )
                            print(
                                f"\033[1;92m[+]\033[0m rdp-brute (CME users): "
                                f"\033[1;92m{rdp_cme_stats.credentials_found} credential(s) found!\033[0m | "
                                f"{rdp_cme_stats.hosts_tested} hosts | "
                                f"{len(new_users)} CME users tested"
                                f"{pwn_str} "
                                f"\033[90m({rdp_cme_stats.scan_time:.1f}s)\033[0m"
                            )
                        else:
                            print(
                                f"\033[92m[+]\033[0m rdp-brute (CME users): "
                                f"\033[37mno valid credentials\033[0m | "
                                f"{rdp_cme_stats.hosts_tested} hosts | "
                                f"{len(new_users)} CME users tested "
                                f"\033[90m({rdp_cme_stats.scan_time:.1f}s)\033[0m"
                        ...
                        print(
                            f"\033[93m[!]\033[0m rdp-brute (CME users): skipped by user\n"
                        )
    ...
    def _run_direct(self, start_time: float) -> ScanResult:
        ...
        # ── RDP brute-force with CME-extracted usernames (direct mode) ───
        if (self.result.cme_available
                and self.result.nmap_available
                and self.result.nmap_results):
            cme_rdp_users = self.cme_scanner.get_rdp_usernames()
            if cme_rdp_users:
                ...
                if new_users:
                    ...
                    rdp_cme_scanner = RDPBruteScanner(self.config.scanner)
                    rdp_cme_output_dir = os.path.join(".", label.replace("/", "_"))
                    os.makedirs(rdp_cme_output_dir, exist_ok=True)
